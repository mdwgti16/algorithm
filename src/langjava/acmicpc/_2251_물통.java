...
public class _2251_물통 {
    ...
    private static void check(int[] waters) {
        if (visited[waters[0]][waters[1]][waters[2]]) {
            return;
        }
            ...
            for (int j = 0; j < 3; j++) {
                ...
                if (waters[i] > 0) {
                    if (waters[j] < capacities[j]) {
                        int[] tempIToJ = moveWater(i, j, k, waters);
                        check(tempIToJ);
                        if (tempIToJ[i] > 0 && waters[k] < capacities[k]) {
                            check(moveWater(i, k, j, tempIToJ));
                        }
                    }
                    if (waters[k] < capacities[k]) {
                        int[] moveIToK = moveWater(i, k, j, waters);
                        check(moveIToK);
                        if (moveIToK[i] > 0 && waters[j] < capacities[j]) {
                            check(moveWater(i, j, k, moveIToK));
                        }
                    }
                }
            }
